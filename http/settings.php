...
if (isset($_GET['rerangefreq']))
{
    if ( is_numeric( $_GET['rerangefreq'] ) )
    { 
        $freq = $_GET['rerangefreq'] ;
        
        if ( $freq >= 0 && $freq <= 500 )
        {
            $settings['userreFreq'] = "true" ;
            $settings['userreRangeFreq'] = $freq;
            ksort($settings);
            writeSettings($settings);
            $sfile = "/opt/minepeon/etc/init.d/miner-start.sh";
            $current = file_get_contents($sfile);
            if ( (strpos( $current, "--rock-freq " ) ) > 0 )
            {
                
                $a = g($current,"--rock-freq ","\n");
                // echo $a ;
                echo "<br>\n";

                $a = str_replace( $a, $_GET['rerangefreq'], $current ) ;
                echo $a ;
                echo "<br>\n";
                echo "<br>\n";
            ...
            echo $current ."<br>\n";
            if ( isset( $_GET['refreqnrestart'] ) )
            {
                // if ( $_GET['refreqnrestart'] == "true" )
                // {
                    // exec('/opt/minepeon/etc/init.d/miner-stop.sh');
                    // sleep( 1 );
                    exec('/opt/minepeon/etc/init.d/miner-restart.sh');
                // }   
            }
            exit;
        }
    }
        // file_put_contents($sfile, preg_replace('/\x0d/', '', $_POST['minerSettings']));
		// exec('/usr/bin/chmod +x /opt/minepeon/etc/init.d/miner-start.sh');
}
// Restore 
...
// Max temp for mkbhitz_plug
if (isset($_POST['miningMxTemp'])) {

  $settings['miningMxTemp'] = $_POST['miningMxTemp'];
  $writeSettings=true;
}
...
<!-- ######################## mkbhitz -->
  ...
  <form name="mkbhitz_minig" action="/settings.php" method="post" class="form-horizontal">
    <fieldset>
      <legend>mkbhitz_minig</legend>
      <div class="form-group">
        <div class="col-lg-9 col-offset-3">
          <div class="checkbox">
            <input type='hidden' value='false' name='mk_amaxTemp'>
            <label>
              <input type="checkbox" <?php echo $settings['mk_amaxTemp']?"checked":""; ?> value="true" id="mk_amaxTemp" name="mk_amaxTemp"> Enable Max Temp
            </label>
          </div>
        </div>
      </div>
	  ...
	  <div class="form-group">
        <div class="col-lg-9 col-offset-3">
          <div class="checkbox" >
            <input type='hidden' value='false' name='mk_adynFreq'>
            <label class="form-group adynFreq-enabled ">
              <input type="checkbox"  class="form-group adynFreq-enabled " <?php echo $settings['mk_adynFreq']?"checked":""; ?> value="true" id="mk_adynFreq" name="mk_adynFreq">mk_adynFreq
            </label>
          </div>
        </div>
      </div>
